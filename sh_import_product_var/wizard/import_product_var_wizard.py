...
class import_product_var_wizard(models.TransientModel):
    ...
    _description = "导入产品变体向导"

    import_type = fields.Selection([
        ('csv', 'CSV 文件'),
        ('excel', 'Excel 文件')
    ], default="csv", string="导入文件类型", required=True)
    file = fields.Binary(string="文件", required=True)

    method = fields.Selection([
        ('create', '创建产品变体'),
        ('write', '创建或更新产品变体')
    ], default="create", string="方法", required=True)

    product_update_by = fields.Selection([
        ('name', '名称'),
        ('barcode', '条码'),
        ('int_ref', '内部参考'),
    ], default='name', string="产品变体更新由", required=True)
...
#         else:
    ...
    def validate_field_selection(self, field_name, field_ttype, field_value, field_required, field_name_m2o):
        ...
        if selection_key_value_list and field_value not in (None, ""):
            ...
            return {"错误": " - " + field_name + " 给定值 " + str(field_value) + " 不符合选择条件. "}
        ...
        for rowx, row in enumerate(map(sheet.row, range(sheet.nrows)), 1):
            ...
            for colx, cell in enumerate(row, 1):
                if cell.ctype is xlrd.XL_CELL_NUMBER:
                    ...
                    raise ValueError(
                        _("行%(row)s, 列%(col)s处的无效单元格值：%(cell_value)s") % {
                            'row': rowx,
                            'col': colx,
                            'cell_value': xlrd.error_text_from_code.get(cell.value, _("未知的错误代码 %s") % cell.value)
                        }
                    )
                else:
                        ...
                        try:
                            ...
                            if row[0] not in (None, "") and row[1] not in (None, ""):
                                ...
                                if row[0] != running_tmpl:
                                    ...
                                    if row[11].strip() == 'Delivered quantities':
                                        tmpl_vals.update(
                                            ...
                                            if "http://" in image_path or "https://" in image_path:
                                                try:
                                                    ...
                                                    if r and r.content:
                                                        ...
                                                        skipped_line_no[str(
                                                            counter)] = " - URL不正确或检查你的图像大小. "
                                                        counter = counter + 1
                                                        continue
                                                except Exception as e:
                                                    skipped_line_no[str(
                                                        counter)] = " - URL不正确或检查你的图像大小 " + ustr(e)
                                                    counter = counter + 1
                                                    continue
                                                ...
                                                try:
                                                    with open(image_path, 'rb') as image:
                                                        ...
                                                        if image_base64:
                                                            ...
                                                            skipped_line_no[str(
                                                                counter)] = " - 找不到图片，或请确保该应用可以访问。 "
                                                            counter = counter + 1
                                                            continue
                                                except Exception as e:
                                                    skipped_line_no[str(
                                                        counter)] = " - 找不到图片，或请确保该应用可以访问。 " + ustr(e)
                                                    counter = counter + 1
                                                    continue
...
#                                         created_product_tmpl.attribute_line_ids = False
                                    ...
                                    if row[14].strip() not in (None, "") and row[15].strip() not in (None, ""):
                                        ...
                                        if self.method == 'create':
                                            ...
                                            if created_product_tmpl.product_variant_ids:
                                                        ...
                                                        if attr_value_price_dic.get(product_varient_attribute_value_id.name, False):
                                                            ...
                                                            if attr_value_price_dic.get(product_varient_attribute_value_id.name) not in [0, 0.0, '', "", None]:
                                                                extra_price = float(attr_value_price_dic.get(
                                                                    product_varient_attribute_value_id.name))

                                                                search_attrs = product_tmpl_attribute_value_obj.search([
                                                                    ('product_tmpl_id', '=',
                                                                     created_product_tmpl.id),
                                                                    ('product_attribute_value_id', '=',
                                                                     product_varient_attribute_value_id.product_attribute_value_id.id),
                                                ...
                                                if product_varient.type == 'product' and row[20] != '':
                                                    stock_vals = {'product_tmpl_id': created_product_tmpl.id,
                                                                  'new_quantity': row[20],
                    ...
                    raise UserError(
                        _("对不起，你的csv或excel文件与我们的格式不一致。" + ustr(e)))

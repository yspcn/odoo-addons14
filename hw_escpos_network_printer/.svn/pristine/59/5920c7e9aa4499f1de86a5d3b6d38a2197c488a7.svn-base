...
"""
    ...
    @http.route(
        "/hw_proxy/scan_item_error_unrecognized", type="json", auth="none", cors="*"
    )
    ...
    @http.route(
        "/hw_proxy/cashier_mode_deactivated", type="json", auth="none", cors="*"
    )
    ...
    def log(self, arguments):
        LOGGER.info(" ".join(str(v) for v in arguments))

...
from . import hw_proxy

try:
    from ..escpos import escpos as Escpos
    from ..escpos import exceptions as E
except ImportError:
    Escpos = Printer = None
...
class EscposDriver(Thread):
    ...
    def get_escpos_printer(self):
        """ to be implemented by network printer classes """
        return None
    ...
    def print_receipt_body(self, eprint, receipt):
        ...
        def quantity(amount):
            if math.floor(amount) != amount:
                return ("{0:." + str(receipt["precision"]["quantity"]) + "f}").format(
                    amount
                )
            else:
                return str(amount)
        ...
        def print_taxes():
            ...
            for tax in taxes:
                eprint.text(
                    printline(
                        tax["tax"]["name"], price(tax["amount"]), width=40, ratio=0.6
                    )
                )
        ...
        for line in receipt["orderlines"]:
            ...
            if (
                ...
                if line["unit_name"] == "Unit(s)":
                    eprint.text(
                        printline(
                            quantity(line["quantity"]) + " x " + price(line["price"]),
                            pricestr,
                            ratio=0.6,
                            indent=2,
                        )
...
class EscposProxy(hw_proxy.Proxy):
    ...
    def open_cashbox(self):
        LOGGER.info("ESC/POS: OPEN CASHBOX")

    #        driver.push_task('cashbox')

    @http.route("/hw_proxy/print_receipt", type="json", auth="none", cors="*")
    def print_receipt(self, receipt):
        LOGGER.info("ESC/POS: PRINT RECEIPT")

    #        driver.push_task('receipt', receipt)

    @http.route("/hw_proxy/print_xml_receipt", type="json", auth="none", cors="*")
    def print_xml_receipt(self, receipt):
        LOGGER.info("ESC/POS: PRINT XML RECEIPT")

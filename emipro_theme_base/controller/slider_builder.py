...
class SliderBuilder(WebsiteSale):
    ...
    def get_product_listing(self, name=False, **kwargs):
        ...
        if name:
            if name == 'new-arrival':
                ...
                products = self.discounted_products('category',request.website.category_check().ids,discount_policy='product',limit=20)
                error = True if not products else False
        ...
        response = http.Response(template='emipro_theme_base.product_display_prod_template',
                                 qcontext={'name': name,'limit':limit, 'products': products, 'filters': filters,'error': error,'error_msg': error_msg})
    ...
    def get_products_of_slider(self, **kw):
        ...
        if product_ids:
            ...
            response = http.Response(template='emipro_theme_base.edit_product_template',qcontext={'products': products})
    ...
    def load_more_category_brand(self,**kw):
        ...
        items= (items - items.filtered(lambda r: r.id not in item_ids)) + items.filtered(lambda r: r.id not in item_ids) if item_ids else items
        ...
        if items:
            ...
            if tmplt:
                response = http.Response(template='emipro_theme_base.list_items',qcontext={'items': items,'name':name})
        return {'response': response.render() if response else False, 'items_count': items_count, 'loaded_items': loaded_items+len(items) if items else loaded_items if loaded_items else 20,}
    ...
    def get_category_brand(self,name,item_ids=False):
        ...
        if name == 'category-slider':
            domain = [('website_id', 'in', [False, request.website.id]), ('image_1920', '!=', False),
                      ('id', 'in', item_ids)] if item_ids else [('website_id', 'in', [False, request.website.id]),('image_1920', '!=', False)]
            items = request.env['product.public.category'].sudo().search(domain,order='id desc')
            items_count = request.env['product.public.category'].sudo().search_count([('website_id', 'in', [False, request.website.id]),('image_1920', '!=', False)])
        else:
            domain = [('website_id', 'in', [False, request.website.id]), ('logo', '!=', False),('id', 'in', item_ids),('website_published', '=', True)] if item_ids else [('website_published', '=', True),('website_id', 'in', [False, request.website.id]),('logo', '!=', False)]
            items = request.env['product.brand.ept'].sudo().search(domain,order='id desc')
            items_count = request.env['product.brand.ept'].sudo().search_count([('website_published', '=', True),('website_id', 'in', [False, request.website.id]),('logo', '!=', False)])
    ...
    def get_brand_category_template(self, **kw):
        ...
        items = False
        item_ids = kw.get('item_ids', False)
        item_ids = [int(i) for i in item_ids.split(',')] if item_ids and type(item_ids) == str else [int(i) for i in item_ids] if item_ids else False
        if name in ['category-slider','brand-slider']:
            ...
            if tmplt:
                response = http.Response(template='emipro_theme_base.brand_category_configure_template',
                                         qcontext={'name': name,'items':items,'items_count':items_count, 'limit':limit, 'styles': styles, 'exclude':exclude, 'loaded_items':loaded_items,'available_slider_style': list(set(styles.mapped('slider_style')))})
                return response.render()
        else:
            tmplt = request.env['ir.ui.view'].sudo().search(
            ...
            if tmplt:
                response = http.Response(template='emipro_theme_base.product_configure_template',
                                         qcontext={'filters': filters,'styles':styles,'exclude':exclude, 'available_slider_style':list(set(styles.mapped('slider_style')))})
                return response.render()
    ...
    def get_suggested_products(self, **kw):
        ...
        products = request.env['product.template'].search([('id', 'not in', exclude_products), ('sale_ok', '=', True),('name', 'ilike', key),('type','in',['product','consu']),('website_published', '=', True)]+website_domain,
                                                          limit=10)
        tmplt = request.env['ir.ui.view'].sudo().search(
    ...
    def best_seller_products(self,limit=10):
        ...
        products = request.env['sale.report'].sudo().browse(sale_report_ids).filtered(lambda r:r.exists()).mapped('product_tmpl_id')
